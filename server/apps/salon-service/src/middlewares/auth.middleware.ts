...
export const attachUser = async (
  ...
  try {
    ...
    if (_id && typeof _id === "string") {
      req.user = { _id, role: (role as string) || "customer" };
      return next();
    }
    ...
    // Direct Cookie fallback
    const tokenCookie = req.cookies?.["access_token"] || req.cookies?.["at"];
    if (tokenCookie) {
      const decoded: any = jwt.decode(tokenCookie);
      if (decoded && (decoded.sub || decoded._id)) {
        req.user = {
          _id: decoded.sub || decoded._id,
          role: decoded.role || "customer",
